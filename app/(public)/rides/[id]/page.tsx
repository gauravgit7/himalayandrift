...
export default async function RideDetailPage({ params }: PageProps) {
  ...
  return (
    ...
    <div className="min-h-dvh pb-20">
      ...
      <section className={`ride-hero-section dark-surface relative bg-gradient-to-b ${heroGradient} pt-24 pb-10 px-4`}>
        ...
        <div className="relative max-w-5xl mx-auto">
          ...
          <Link
            ...
            className="print:hidden inline-flex items-center gap-1.5 text-sm text-tvs-charcoal-400 hover:text-tvs-charcoal-100 transition-colors mb-6"
          >
            <ArrowLeft className="size-4" />
            Back to Calendar
          </Link>

          {/* Print-only header: TVS Nepal branding */}
          <div className="hidden print:flex items-center justify-between mb-4 pb-2 border-b border-gray-300">
            <span className="text-base font-black tracking-tight text-gray-900">TVS Nepal</span>
          ...
          {/* Meta row */}
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-tvs-charcoal-400 print:text-gray-700">
            <span className="flex items-center gap-1.5">
              <Calendar className="size-3.5 shrink-0" />
              <span className="flex flex-col leading-tight">
                <span>{adDateLabel}</span>
                <span className="text-xs text-tvs-charcoal-600 print:text-gray-500">
                  {bsDateLabel}
                </span>
              </span>
            </span>
          ...
          <div className="print:hidden mt-6 flex flex-wrap items-center gap-3">
            {hasRegistration && (
              <a
                ...
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-tvs-red-600 hover:bg-tvs-red-500 text-white font-bold text-sm shadow-glow-red transition-all hover:scale-[1.02] active:scale-[0.98]"
              >
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2.5 rounded-xl bg-tvs-charcoal-800/80 border border-tvs-charcoal-700 text-tvs-charcoal-300 hover:text-white hover:bg-tvs-charcoal-700 hover:border-tvs-charcoal-500 text-xs font-semibold transition-all"
      ...
      {/* ── Stats bar ─────────────────────────────────────────────────────── */}
      <section className="print:hidden border-b border-tvs-charcoal-800/60 bg-tvs-charcoal-950/80">
        <div className="max-w-5xl mx-auto px-4 py-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              <div
                key={label}
                className="flex items-center gap-3 p-3 rounded-xl bg-tvs-charcoal-900/60 border border-tvs-charcoal-800/60"
              >
                <span className="text-tvs-charcoal-500 shrink-0">{icon}</span>
                <div>
                  <p className="text-[10px] text-tvs-charcoal-600 uppercase tracking-wide">{label}</p>
                  <p className="text-sm font-semibold text-tvs-charcoal-50">{value}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="max-w-5xl mx-auto px-4 pt-8">
        <div className="ride-main-grid grid lg:grid-cols-[1fr_340px] gap-8">
          ...
          <div className="space-y-8">
            ...
            {ride.description && (
              <div>
                <h2 className="text-xs font-bold uppercase tracking-widest text-tvs-charcoal-500 mb-3">
                  About this Ride
                </h2>
                <p className="text-tvs-charcoal-300 leading-relaxed text-[15px]">
                  {ride.description}
                </p>
              </div>
            )}
            ...
            {hasItinerary && (
              <div>
                <h2 className="text-xs font-bold uppercase tracking-widest text-tvs-charcoal-500 mb-4">
                  ...
                  {ride.itinerary.map((day, idx) => (
                    <div
                      ...
                      className={cn(
                        ...
                        "border border-tvs-charcoal-700/60"
                      )}
                    >
                      {/* Day badge */}
                      <div className="shrink-0 flex flex-col items-center">
                        <div className="size-8 rounded-full bg-tvs-charcoal-800 border border-tvs-charcoal-700 flex items-center justify-center text-xs font-bold text-tvs-charcoal-300">
                          {day.day}
                        </div>
                        {idx < ride.itinerary.length - 1 && (
                          <div className="w-px flex-1 bg-tvs-charcoal-800 mt-1.5" />
                        )}
                      </div>
                      {/* Content */}
                      <div className="flex-1 min-w-0 pb-2">
                        <p className="font-semibold text-tvs-charcoal-50 text-sm">{day.title}</p>
                        {(day.startLocation || day.endLocation) && (
                          <p className="flex items-center gap-1 text-xs text-tvs-charcoal-400 mt-0.5">
                            <span>{day.startLocation}</span>
                            <ChevronRight className="size-3 shrink-0" />
                            <span>{day.endLocation}</span>
                            {day.estimatedKm != null && day.estimatedKm > 0 && (
                              <span className="ml-1 text-tvs-charcoal-600">
                                · {day.estimatedKm} km
                              </span>
                            )}
                          </p>
                        ...
                        {day.description && (
                          <p className="text-xs text-tvs-charcoal-400 mt-1.5 leading-relaxed">
                            {day.description}
                          </p>
                        )}
                      </div>
            ...
            {ride.tags.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {ride.tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-xs px-2.5 py-1 rounded-full bg-tvs-charcoal-800/60 border border-tvs-charcoal-700/40 text-tvs-charcoal-400"
                  >
                    #{tag}
                  </span>
                ))}
          ...
          <div className="ride-sidebar space-y-5">
            ...
            {hasRegistration && (
              <div className={cn("print:hidden p-5 rounded-xl gradient-card border", accentBorder)}>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 w-full px-4 py-2.5 rounded-lg bg-tvs-red-600 hover:bg-tvs-red-500 text-white font-bold text-sm transition-all hover:shadow-glow-red"
                >
                  <Flag className="size-4" />
                  Register
                  <ExternalLink className="size-3.5 opacity-70" />
            ...
            {hasRoute && (
              <div className="print:hidden p-4 rounded-xl gradient-card border border-tvs-charcoal-700/60">
                <div className="flex items-center gap-2 mb-3">
                  <Navigation className="size-4 text-tvs-charcoal-400" />
                  <h3 className="text-sm font-bold text-tvs-charcoal-50">Route Map</h3>
                  <span className="ml-auto text-xs text-tvs-charcoal-500">
                ...
                <ol className="mt-3 space-y-1.5">
                  {ride.routeData!.waypoints.map((wp, i) => (
                    <li key={i} className="flex items-center gap-2 text-xs text-tvs-charcoal-400">
                      <span className="size-5 rounded-full bg-tvs-charcoal-800 border border-tvs-charcoal-700 flex items-center justify-center text-[9px] font-bold text-tvs-charcoal-300 shrink-0">
                        {i + 1}
                      </span>
                      <span className={i === 0 || i === ride.routeData!.waypoints.length - 1
                        ? "font-semibold text-tvs-charcoal-200"
                        : ""}
                      >
                        {wp.name}
                      </span>
            ...
            {isUpcoming && (
              <div className="print:hidden p-4 rounded-xl gradient-card border border-tvs-charcoal-700/60">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-sm font-bold text-tvs-charcoal-50">
                    Destination Weather
                  </h3>
                </div>
                ...
                {weather ? (
                  <>
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        ...
                        <div className="flex items-baseline gap-1 mt-0.5">
                          ...
                          <span className="text-sm text-tvs-charcoal-400">C</span>
                        </div>
                        <p className="text-xs text-tvs-charcoal-400 capitalize mt-0.5">
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-3">
                      {[
                        ...
                        <div key={label} className="p-2 rounded-lg bg-tvs-charcoal-800/60 border border-tvs-charcoal-700/40">
                          <p className="text-[9px] text-tvs-charcoal-600 uppercase tracking-wide">{label}</p>
                          <p className={cn(
                            "text-xs font-semibold mt-0.5",
                            label === "Riding"
                              ? CONDITION_META[weather.ridingCondition].color
                              : "text-tvs-charcoal-200"
                          )}>
                            {value}
                          </p>
                        </div>
                    ...
                    {weather.forecast.length > 0 && (
                      <div className="flex gap-1 pt-2 border-t border-tvs-charcoal-800/60">
                        {weather.forecast.map((day) => (
                          <div key={day.date} className="flex-1 text-center py-1">
                            <p className="text-[9px] text-tvs-charcoal-500">
                  ...
                  <p className="text-xs text-tvs-charcoal-500 py-2">
            ...
            {/* Marshal */}
            {ride.marshal && (
              <div className="p-4 rounded-xl gradient-card border border-tvs-charcoal-700/60">
                <p className="text-[10px] font-bold uppercase tracking-wide text-tvs-charcoal-500 mb-2">
                  Lead Marshal
                </p>
                <div className="flex items-center gap-3">
                  <div className="size-9 rounded-full bg-tvs-charcoal-800 border border-tvs-charcoal-700 flex items-center justify-center text-sm font-bold text-tvs-charcoal-300 shrink-0">
                    {ride.marshal.name.charAt(0)}
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-tvs-charcoal-50">{ride.marshal.name}</p>
                    {ride.marshal.phone && (
                      <p className="text-xs text-tvs-charcoal-500">{ride.marshal.phone}</p>
                    )}
                  </div>
                </div>
              </div>
            ...
            {ride.sponsors.length > 0 && (
              <div className="print:hidden p-4 rounded-xl gradient-card border border-tvs-charcoal-700/60">
                <p className="text-[10px] font-bold uppercase tracking-wide text-tvs-charcoal-500 mb-3">
                  Supported by
                </p>
                <div className="flex flex-wrap gap-2">
                  {ride.sponsors.map((s) => (
                    <div
                      key={s.id}
                      className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-tvs-charcoal-800/60 border border-tvs-charcoal-700/40"
                    >
                      <div className="size-4 rounded bg-tvs-charcoal-700 flex items-center justify-center text-[8px] font-bold text-tvs-charcoal-400">
                        {s.name.charAt(0)}
                      </div>
                      <span className="text-xs text-tvs-charcoal-300">{s.name}</span>
                    </div>
                  ))}
                </div>
              </div>
